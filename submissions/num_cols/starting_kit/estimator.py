...
def fold_time_series(time_point, period, div_period):
    return (
        time_point - (time_point // (period / div_period)) * period / div_period
    )
...
transformer_r = FunctionTransformer(lambda X_df: get_bin_means(X_df, 5, "r"))

transformer_b = FunctionTransformer(lambda X_df: get_bin_means(X_df, 5, "b"))
